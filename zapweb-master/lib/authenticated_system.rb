module AuthenticatedSystem
  protected
    ...
    # Allows a user to be logged in if they have a token (cookie) that matches
    # the saved token in the user model  
    # def login_from_cookie
    #   return unless cookies[:auth_token] && session[:user_id].nil?
    #   user = User.find_by_remember_token(cookies[:auth_token])
    #   return if user.nil?
    #   return if user.must_change_pwd 
    ...
    def send_remember_cookie!
      cookies[:auth_token] = {
        ...
        :expires => @current_user.remember_token_expires }
    end
